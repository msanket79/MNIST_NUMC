...
template<typename TP>
inline void kernelScalerSubMat(const TP*A,const TP b,TP*C,const int rows,const int cols);
...
//maximum-------------------
template<typename TP>
inline void kernelMatMaximumScaler(const TP* A,const TP b, TP*C,const int rows,const int cols);
template<typename TP>
...
//minimum-------------------
template<typename TP>
inline void kernelMatMinimumScaler(const TP* A,const TP b, TP*C,const int rows,const int cols);
template<typename TP>
...
template<typename TP>
inline void kernelVecGreaterThanEqualMat(const TP* A, const TP b, TP* C, const int rows, const int cols) {
#pragma omp parallel for
	for (int i = 0; i < rows; i++) {
		for (int j = 0; j < cols; j++) {
			C[i * cols + j] = (A[i * cols + j] <= b ? 1 : 0);
...
template<typename TP>
inline void kernelScalerLessThanMat(const TP* A, const TP b, TP* C, const int rows, const int cols) {
#pragma omp parallel for
	for (int i = 0; i < rows; i++) {
		for (int j = 0; j < cols; j++) {
			C[i * cols + j] = (A[i * cols + j] > b ? 1 : 0);
