...
public:
    ...
    ArrayCpu<TP> at(const ArrayCpu<int>&rowsIndices,const ArrayCpu<int>&olumnIndices);

    void set(const ArrayCpu<TP>&rowsIndices,const ArrayCpu<TP>&columnIndices,const ArrayCpu<TP>&values);
...
template<typename TP>
TP np::ArrayCpu<TP>::at(int i, int j) {

    return mat[i * cols + j];
}

template<typename TP>
ArrayCpu<TP> ArrayCpu<TP>::at(const ArrayCpu<int>&rowsIndices,const ArrayCpu<int>&columnIndices){
    auto A=ArrayCpu<TP>(1,rowsIndices.rows);
    for(int i=0;i<A.cols;i++){
        A.mat[0]=this->at(rowsIndices[i],columnIndices[i]);
    }
    return A;
}

template<typename TP>
void ArrayCpu<TP>::set(const ArrayCpu<TP>&rowsIndices,const ArrayCpu<TP>&columnIndices,const ArrayCpu<TP>&values){
    for(int i=0;i<A.cols;i++){
        this->set(rowsIndices[i],columnIndices[i],values[i]);
    }
}
