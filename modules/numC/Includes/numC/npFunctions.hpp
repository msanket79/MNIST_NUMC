...
namespace np {
	...
	ArrayCpu<TP> arrange(const TP stop);
	...
	template<typename TP>
	ArrayCpu<TP> log( const ArrayCpu<TP>&A);
...
//function declaration-------------
	...
	ArrayCpu<TP> maximum(ArrayCpu<TP>& A,TP b){
		...
		kernelMatMaximumScaler<TP>(A,b,C,A.rows,B.rows);
		return C;
	}
//--------------------------------------------
	...
	np::ArrayCpu<TP> minimum(np::ArrayCpu<TP>&A,np::ArrayCpu<TP>& B) {
		...
		if (this->rows == 1 and this->cols == 1) {
			...
			auto C = ArrayCpu<TP>(B.rows, B.cols);
			kernelMatMinimumScaler<TP>(B.mat, this->mat[0], C.mat, B.rows, B.cols);
			return C;
			...
			auto C = ArrayCpu<TP>(this->rows, this->cols);
			kernelMatMinimumScaler<TP>(this->mat, B(0, 0), C.mat, this->rows, this->cols);
			return C;
		}
		else if (this->rows == B.rows && this->cols == B.cols) {

			auto C = ArrayCpu<TP>(this->rows, this->cols);
			kernelMatMinimumMat<TP>(this->mat, B.mat, C.mat, rows, cols);
			return C;
		}
		else if (this->rows == 1 || this->cols == 1) {
			...
			if (vecDim == B.rows && vecDim == B.cols) {
				...
				auto C = ArrayCpu<TP>(B.rows, B.cols);
				kernelMatMinimumVecAlongCols<TP>(B.mat, this->mat, C.mat, B.rows, B.cols);
				return C;
			}
			else if (vecDim == B.cols) {
				auto C = ArrayCpu<TP>(B.rows, B.cols);
				kernelMatMinimumVecAlongRows<TP>(B.mat, this->mat, C.mat, B.rows, B.cols);
				return C;
			}
		}
		else if (B.rows == 1 || B.cols == 1) {
			...
			if (vecDim == this->rows && vecDim == this->cols) {
				...
				auto C = ArrayCpu<TP>(this->rows, this->cols);
				kernelMatMinimumVecAlongCols<TP>(this->mat, B.mat, C.mat, this->rows, this->cols);
				return C;
			}
			else if (vecDim == this->cols) {
				auto C = ArrayCpu<TP>(this->rows, this->cols);
				kernelMatMinimumVecAlongRows<TP>(this->mat, B.mat, C.mat, this->rows, this->cols);
				return C;
			}
		}
	...
	ArrayCpu<TP> minimum(ArrayCpu<TP>& A,TP b){
		auto C=ArrayCpu<TP>(A.rows,A.cols);
		kernelMatMinimumScaler<TP>(A,b,C,A.rows,B.rows);
		return C;
	}
